...
echo "\nparse earthquake.geojson\n"
./$VMDIR/pegvm -t stat -p $BYTECODE/json_noObj.bin -i $DATADIR/earthquake.geojson

echo "\nparse benchmark4.json\n"
./$VMDIR/pegvm -t stat -p $BYTECODE/json_noObj.bin -i $DATADIR/benchmark4.json

echo "\nparse xmark5m.xml\n"
./$VMDIR/pegvm -t stat -p $BYTECODE/xml_noObj.bin -i $DATADIR/xmark5m.xml

echo "\nparse nss_cache.c\n"
./$VMDIR/pegvm -t stat -p $BYTECODE/c99_noObj.bin -i $DATADIR/nss_cache.c